...
<%! final private static Logger log = LoggerFactory.getLogger("uploadResume_JSP"); %>
<%
	...
	<div class=" w3l-login-form">
		<%
			...
			try {
				...
				String sql = "select resume from student where roll_no = '" + session.getAttribute("ROLL_NO") + "'";
				rs = statement.executeQuery(sql);
				if (rs.next()) {
					fileName = rs.getString("resume");
					if (fileName != null)
						fileName = "/Download?file=" + fileName + "&type=application/pdf";
				}
			} catch (Exception e) {
				log.error("Could not upload resume : {}", e);
			}
		%>
		<center>
			<p style="color: white">
				<%
					if (fileName != null) {
				%>Download Resume <a href="<%=fileName%>"
		...
		<form action="UploadResume" method="POST"
			...
			<div class=" w3l-form-group">
				...
				<div class="group">
					<input id="pdf-file" type="file" class="form-control"
						placeholder="Upload Resume" name="filename" required="required"
						size="50" accept="application/pdf" />
					<script type="text/javascript">
        $('#pdf-file').bind('change', function() {
