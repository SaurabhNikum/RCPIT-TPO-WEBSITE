...
public class UploadResume extends HttpServlet {
	...
	final private static Logger log = LoggerFactory.getLogger(UploadResume.class);
	/**
			...
			try {
				...
				while (i.hasNext()) {
					...
					if (!fi.isFormField()) {
						...
						PreparedStatement ps = conn.prepareStatement("update student set resume = ? where student_id = ?");
				...
				log.error("Could not add resume : {}", ex);
